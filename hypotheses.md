...
- H3 — Constructor has no zero-address check on airdropToken and no zero-value check on merkleRoot. If deployed with either as zero, the contract is permanently unusable and must be redeployed.
  Location: constructor, MerkleAirdrop.sol
  Severity guess: Low (deployment footgun, not exploitable post-deploy)
